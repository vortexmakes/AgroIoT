...
#define calc_check(s) \
    (ushort)(((((s & CHECK_MASKH) << 4) + (CHECK_OFFSET << 8))) \
...
#define TPLINK_COMFLOW_MSK      0x80                    /* Com flow mask, Master
                                                         * to Slave */
...
#if ((TPLINK_SLAVE_ADDRESS == TPLINK_SOF) || \
    (TPLINK_SLAVE_ADDRESS == TPLINK_ESC))
