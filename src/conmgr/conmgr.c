...
RKH_DCLR_BASIC_STATE ConMgr_inactive, ConMgr_sync,
                     ConMgr_init, ConMgr_pin, ConMgr_setPin,
                     ...
                     ConMgr_checkIP, ConMgr_waitRetryConfig,
                     ConMgr_waitingServer,
                     ConMgr_idle, ConMgr_getStatus, ConMgr_waitPrompt,
                     ConMgr_waitOk,
...
RKH_CREATE_BASIC_STATE(ConMgr_cipShutdown,
                       cipShutdown,
                       NULL,
                       &ConMgr_initialize,
                       NULL);
...
RKH_CREATE_COMP_REGION_STATE(ConMgr_registered,
                             regEntry,
                             regExit,
                             &ConMgr_active,
...
RKH_CREATE_BASIC_STATE(ConMgr_failure,
                       failureEntry,
                       failureExit,
                       &ConMgr_active,
                       NULL);
...
unregEntry(ConMgr *const me)
{
    ...
    RKH_TMR_ONESHOT(&me->timerReg, RKH_UPCAST(RKH_SMA_T, me),
                    REGISTRATION_TIME);
}
