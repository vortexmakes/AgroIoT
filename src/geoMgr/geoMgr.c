...
static const unsigned char rmcHz[] = UBX_RMC_1SEC;
...
publishRmc(GeoMgr *const me, RKH_EVT_T *pe)
{
    ...
    *pGps->longInd = *(pRmc->eastingIndicator) == 'E' ? '+' : '-';
    ...
    topic_publish(Status, RKH_UPCAST(RKH_EVT_T, geoEvt), 
...
publishInvRmc(GeoMgr *const me, RKH_EVT_T *pe)
{
    ...
    topic_publish(Status, RKH_UPCAST(RKH_EVT_T, &geoInvalidEvt), 
                          RKH_UPCAST(RKH_SMA_T, me));
}
...
turnsDetect(GeoMgr *const me)
{
        ...
        (currSog > Config_getAccLimit()))
    {
        ...
        {
            topic_publish(Status, RKH_UPCAST(RKH_EVT_T, &turnEvt), 
                                  RKH_UPCAST(RKH_SMA_T, me));
        }
    }
}
